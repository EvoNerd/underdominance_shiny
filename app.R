...
ui <- fluidPage(pageWithSidebar( 
  ...
  sidebarPanel(
    ...
    HTML("<p style='font-size:14px'><B>Fitness of AA (wAA)= 1.1<BR>
    ...
    sliderInput(inputId = "p_0", label = "Initial p", value = 0.38, 
...
#              text = element_text(size=16, family= "Times"))
      ...
      data.frame(t = 1:gen, p_t = p_t) %>%
        ggplot(aes(x = t-1)) +
        scale_color_manual(values = c("genotype AA" = "firebrick", "genotype Aa" = "orange", "genotype aa" = "gold")) +
        geom_point(aes(y = (1-p_t)^2, color="genotype aa"), size=1) +
        geom_point(aes(y = 2*p_t*(1-p_t), color="genotype Aa"), size=1) +
        geom_point(aes(y = p_t^2, color="genotype AA"), size=1) +
