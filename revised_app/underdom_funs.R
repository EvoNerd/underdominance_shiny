...
colour_p <- "#21908C" # previous colour: "#2D1C56"
...
sim_forward_time <- function(num_gens, freq_init, w_genotypes){
  ...
  # finally, create a data.frame for the genotype frequencies over time
...
ggplot_genotype_finite <- function(genotypes_df, genotype_levels){
  # get x-axis labels
  tickwidth_x <- floor(max(genotypes_df$gens)/4)
...
ggplot_p_finite <- function(p_df){
  # get x-axis labels
  tickwidth_x <- floor(max(p_df$gens)/4)
  labs_x <- seq(from=tickwidth_x, to=3*tickwidth_x, by=tickwidth_x)
