...
*",html_col1,"Try sliding $h$ to see how the fitness of the heterozygotes changes on the fitness landscape (top left).</span>
...
*",html_col1,"Try sliding $p_0$ to see how the genotype frequencies change over time (top right).</span>
...
",html_col1,"*How do the plots on the right change when you adjust the third slider?*</span>")
...
",html_col1,"**Key insight:**</span> When $A$ is rare, almost all $A$ alleles are in heterozygote genotypes. If heterozygotes are unfit ($h<0$), $A$ will not spread when rare -- even if $AA$ is very fit!")
...
- ",html_col1,"Set $h$ below 0:</span> $A$ can spread only when it starts from a sufficiently common initial frequency $p_0$.
